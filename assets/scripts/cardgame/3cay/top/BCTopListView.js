    ...
    cc.BCTopListView = cc.Class({
        ...
        initialize: function (messages) {
            ...
            for (var i = 0; i < spawnCountReal; ++i) {
                ...
                item.setPosition(0, -item.height * (0.5 + i) - this.spacing * (i + 4));
                item.getComponent(cc.BCTopItem).updateItem(this.messages[i], i);
                this.items.push(item);
            }
            this.rootContentY = this.content.y*300;
        },
            ...
            for (var i = 0; i < items.length; ++i) {
                ...
                if (isDown) {
                    ...
                    if (viewPos.y < -buffer && items[i].y + offset < 0) {
                        ...
                        var item = items[i].getComponent(cc.BCTopItem);
                    ...
                    if (viewPos.y > buffer && items[i].y - offset > -this.content.height) {
                        items[i].y  = (items[i].y - offset);
                        item = items[i].getComponent(cc.BCTopItem);
