...
(function () {
    cc.PKPlayer = cc.Class({
        ...
        updateStatus: function (status) {
            if(status == "BỎ")
            {
                  this.bgStt.spriteFrame = this.spBg[2];
            }
            if(status == "THEO")
            {
                  this.bgStt.spriteFrame = this.spBg[0];
            }
            if(status == "XEM")
            {
                  this.bgStt.spriteFrame = this.spBg[1];
            }
            if(status == "XẢ LÁNG")
            {
                  this.bgStt.spriteFrame = this.spBg[3];
            }
            if(status == "THÊM")
            {
                  this.bgStt.spriteFrame = this.spBg[4];
            }
            // this.bgStt.spriteFrame = this.spBg[1];
            this.bgStt.node.active = true;
