...
(function () {
    ...
    Tool = (function () {
        ...
        Tool.prototype.formatMoneyToKMB = function(money, isForMatK = true) {
            let format = "";
        ...
        Tool.prototype.formatNumberSicbo = function (num, digits = 3) {
        ...
        Tool.prototype.formatNumberTaiXiu = function (value) {
            var newValue = value;
            if (value >= 1000) {
                var tempValue = value;
                var suffixNum = 0;
                tempValue = Math.floor(tempValue/1000);
                while(tempValue > 999999)
                {
                    tempValue = Math.floor(tempValue/1000);
                    suffixNum++;
                }
                ...
                var shortValue = '';
                shortValue = parseInt(tempValue).toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.");
        ...
        Tool.prototype.formatWalletMoney = function (num, digits = 2) {
            if (isNaN(parseFloat(num))) return 0;
            const si = [
                { value: 1, symbol: "" },
                { value: 1E3, symbol: "K" },
        ...
        //(dd + 1)/mm/yyyy hh:mm:ss //Kiểm tra năm nhuận, các tháng
        Tool.prototype.getLocalDateNow4 = function (hoursAgo) {
            var d = new Date();
