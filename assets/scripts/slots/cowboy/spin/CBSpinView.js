...
(function () {
    cc.CBSpinView = cc.Class({
        extends: cc.SpinViewBase,
        properties: {
            spinColumnViews: [cc.CBSpinColumnView],
        ...
        onLoad: function () {
            ...
            this.listAutoSpin = [5,10,25,50,100,200,500,1000,2000,5000];
            this.currentASpinIndex = 9;
            this.currentASpinCount = this.listAutoSpin[this.currentASpinIndex];
            this.lbAutoSpinCount.string = this.currentASpinCount;
        ...
        btnNextLines(){
            ...
            this.updateBetLine(this.currentLine);
            cc.PayLinesController.getInstance().activeLines(this.currentLine);
        ...
        btnBackLines(){
            ...
            if(this.currentLine == 0 ){
                this.currentLine = this.totalLine;
            }
            cc.PayLinesController.getInstance().activeLines(this.currentLine);
            this.updateBetLine(this.currentLine)
        ...
        updateBetLine(currentLine = 1){
            ...
            this.updateBetLinesText(betLinesText);
        },

        //update tong so Line UI
        updateTotalLines: function (totalLines) {
            this._super(totalLines);
            this.currentLine = totalLines;
        },


        btnNextASpin(){
            this.currentASpinIndex++;
            if(this.currentASpinIndex == this.listAutoSpin.length){
                this.currentASpinIndex = 0;
            }
            this.currentASpinCount = this.listAutoSpin[this.currentASpinIndex];
            this.lbAutoSpinCount.string = this.currentASpinCount;
        },
        ...
        btnBackASpin(){
            this.currentASpinIndex--;
            if(this.currentASpinIndex == -1 ){
                this.currentASpinIndex = this.listAutoSpin.length - 1;
            }
            this.currentASpinCount = this.listAutoSpin[this.currentASpinIndex];
            this.lbAutoSpinCount.string = this.currentASpinCount;
        },
        ...
        updateRoom(room = 0){
            ...
            if (roomId === 0) {
                this.roomController.sendRequestOnHub(cc.MethodHubName.PLAY_TRY);
        ...
        onEnable: function () {
            this._super();
            this.lbNickName.string = cc.LoginController.getInstance().getNickname();
            this.sound = true;
        ...
        stopSpinFinish: function () {
            ...
            if (this.indexStopFinish === 5) {
                ...
                this.playEffect(this.spinData, this.spinResponse.AccumulateGame);

                //set diem thich luy FreeSpin
                cc.CBAccumulateController.getInstance().onUpdateAccumulate(this.spinResponse.AccumulateGame);

            }
        },


        playEffect: function (spinData, accumulateGame) {
            var self = this;
            ...
            if (spinData.IsJackpot) {
                ...
                this.spinController.activeButtonAutoSpinMain(!this.isAutoSpin);
                timeWaitJackpot = 5;
            }
            if (parseInt(accumulateGame.PrizeValue) > 0) {
                timeWaitJackpot = 8;
            }
            ...
            this.scheduler.schedule(function () {
                ...
                if (cc.FreeSpinController.getInstance().getStateFreeSpin() === false && spinData.TotalFreeSpin > 0) {
                    //cap nhat trang thai freeSpin
                    cc.FreeSpinController.getInstance().setStateFreeSpin(true);
        ...
        checkHaveFreeSpin: function (spinData) {
            ...
            if (cc.FreeSpinController.getInstance().getStateFreeSpin() && spinData.TotalFreeSpin === 0) {
                ...
                }, this, 0, 0, self.isFastSpin ? slotsConfig.TIME_WAIT_LOST_FAST : slotsConfig.TIME_WAIT_RESULT_FREE_SPIN, false);
            } else {
                ///Neu tu dong quay thi tu goi SPIN tiep
                if (self.isAutoSpin && (self.bonusGameData === undefined || self.bonusGameData.BonusData === null)) {
                    self.updateAutoSpin();
        ...
        updateAutoSpin(){
            this.currentASpinCount--;
            if( this.currentASpinCount == -1){
            ...
            if (!cc.FreeSpinController.getInstance().getStateFreeSpin()) {
                if ((!cc.SpinController.getInstance().getIsPlayTry()
                    ...
                    && cc.BalanceController.getInstance().getTryBalance() < this.totalBet)) {
                        cc.PopupController.getInstance().showSlotsMessage('Không đủ số dư');
                    //tat auto SPIN
                    this.isAutoSpin = false;
                    this.spinController.activeButtonAutoSpin(this.isAutoSpin);
                    //bat lai button SPIN
                    this.spinController.activeButtonSpin(true);
                    this.spinController.activeButtonAutoSpinMain(true);
                    return false;
                }  else {
                    return true;
                }
