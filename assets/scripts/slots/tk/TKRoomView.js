...
(function() {
    cc.TKRoomView = cc.Class({
        "extends": cc.RoomView,
        properties: {
            lbNickName: cc.Label,
            lbBalance: cc.LabelIncrement,
            lineFrame1: [cc.SpriteFrame],
            lineFrame2: [cc.SpriteFrame],
            lineFrame3: [cc.SpriteFrame],
        ...
        getX6JackpotInfo() {
            ...
            gameListX6JackpotInfo.forEach((jackpotInfo) => {
                ...
                if (jackpotInfo.GameID.toString() === cc.GameId.THREE_KINGDOM) {
        ...
        resetAllJackpotEffects() {
            this.xJPVuongQuoc.forEach(item => {
                if (item) {
        ...
        activeMain: function () {
            ...
            //Tắt màn hình chọn level ở sảnh
            this.nodeRoom.getChildByName("jackpotView").active = false;
            this.nodeRoom.active = true;
        },
        ...
        updateUIByRoomID(roomId){
            ...
            let playLine = this.nodeMain.children[0].children;
            this.nodeRoom.getChildByName("ObjectLevel").getChildByName("ObjectLevel1").active = false;
            this.nodeRoom.getChildByName("ObjectLevel").getChildByName("ObjectLevel2").active = false;
            this.nodeRoom.getChildByName("ObjectLevel").getChildByName("ObjectLevel3").active = false;
            cc.PayLinesController.getInstance().hideAllLines();
            switch(roomId) {
                case 1: 
                    this.nodeRoom.getChildByName("ObjectLevel").getChildByName("ObjectLevel1").active = true;
                    for (let i = 0; i < playLine.length; i++) {
                        playLine[i].getComponent(cc.Sprite).spriteFrame = this.lineFrame1[i]
                    }
                break;
                case 2:
                    this.nodeRoom.getChildByName("ObjectLevel").getChildByName("ObjectLevel2").active = true;
                    for (let i = 0; i < playLine.length; i++) {
                        playLine[i].getComponent(cc.Sprite).spriteFrame = this.lineFrame2[i]
                    }
                break;
                default:
                    this.nodeRoom.getChildByName("ObjectLevel").getChildByName("ObjectLevel3").active = true;
                    for (let i = 0; i < playLine.length; i++) {
                        playLine[i].getComponent(cc.Sprite).spriteFrame = this.lineFrame3[i]
                    }
                break;
            }
        },
        ...
        if (response.M !== undefined && response.M.length > 0) {
            ...
            switch (m.M) {
                ...
                case cc.MethodHubOnName.JOIN_GAME:
                    ...
                    this.updateUIByRoomID(data.RoomID);
                    switch (this.gameId) {
                        case cc.GameId.THREE_KINGDOM:
                            if(data.RoomID == 1 || data.RoomID == 3){
