...
(function () {
    cc.BonusGameView = cc.Class({
        ...
        properties: {
            ...
            lbTime: cc.Label,
            lblBonusMultiWin: cc.LabelIncrement,
        ...
        update: function (dt) {
            if (this.isTimer) {
                this.timer += dt;
                this.lbTime.string = `00:${Math.round(slotsConfig.TIME_WAIT_AUTO_FINISH_BONUS_GAME - this.timer)}`;
                ...
                if (this.timer >= slotsConfig.TIME_WAIT_AUTO_FINISH_BONUS_GAME) {
                    if (cc.BonusGameController.getInstance().getBonusGameState() !== cc.BonusGameState.MULTI) {
                        ...
                        cc.RoomController.getInstance().sendRequestOnHub(cc.MethodHubName.PLAY_BONUS, 12, 1);
                    } else {
