...
var slotsConfig = require('ThuyCungConfig');
var gameMessage = require('GameMessage');

(function () {
    cc.ThuyCungSpinView = cc.Class({
        extends: cc.SpinViewBase,
        properties: {
            spinColumnViews: [cc.ThuyCungSpinColumnView],
            //anh nen cua phong
        ...
        btnNextRoom(){
            cc.AudioController.getInstance().playSound(cc.AudioTypes.INCREASE_BET);
            if( cc.SpinController.getInstance().getIsPlayTry()){
        ...
        btnBackRoom(){
            cc.AudioController.getInstance().playSound(cc.AudioTypes.DECREASE_BET);
            if( cc.SpinController.getInstance().getIsPlayTry()){
                cc.PopupController.getInstance().showSlotsMessage('Bạn vui lòng vào phòng chơi để thực hiện chức năng này!');
            } else {
                this.currentIndex--;
        ...
        updateBGRoomUI: function () {
            // this.listRoomID = [1,2,4];
            let roomID = cc.RoomController.getInstance().getRoomId()
            let index = this.listRoomID.findIndex(itemID => itemID == roomID);
            if(index >=0){
        ...
        callSpin: function () {
            ...
            cc.AudioController.getInstance().playSound(cc.AudioTypes.START_SPIN);
        ...
        playEffect: function (spinData) {
            ...
            this.scheduler.schedule(function () {
                ...
                if (spinData.PaylinePrize > 0) { //PaylinePrize //TotalPrize
                    if (!spinData.IsJackpot) {
                        if (spinData.PaylinePrize >= (this.spinResponse.BetValue * this.spinResponse.TotalLine) * 100 ) {
