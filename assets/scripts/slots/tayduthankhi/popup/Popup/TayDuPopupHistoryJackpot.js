...
cc.Class({
    ...
    updateDataByPageIndex(pageIndex){
        ...
        this.listItem.forEach((item,index)=>{
            if(result[index]){
                ...
                item.getChildByName("lbNickName").getComponent(cc.Label).string = result[index].UserName;
                item.getChildByName("lbType").getComponent(cc.Label).string = result[index].JackpotType==1?"Minor":result[index].JackpotType==2?"Major":"Grand";
            } else {
                item.active = false;
            }
        });
    ...
    show () {
        this._super();
        this.clickButtonFirstPage();
        let self = this;
        let url = 'api/tayduthankhi/GetBigWinner?top=100&type=' + cc.BigWinnerType.JACKPOT;
        cc.ServerConnector.getInstance().sendRequest(cc.SubdomainName.TAY_DU_THAN_KHI, url,  (response) =>{
            let obj = JSON.parse(response);
            self.onGetJackpotResponse(obj)
        });
        this.buttonFirst.interactable = false;
        this.buttonBack.interactable = false;
        this.buttonLast.interactable = false;
