...
export default class Play extends cc.Component {
        ...
        this.toggleAuto.node.on("toggle", () => {
            ...
            if (this.toggleAuto.isChecked) {
                ...
                this.fishs.forEach(fish =>{
                    fish.getComponent(cc.Button).enabled = true;
                    fish.node.off("click");
                    fish.node.on("click", () => {
                        if(this.isShoot){
                            this.countShoot = 0;        
                ...
                this.stopAutoShoot();
                this.fishs.forEach(fish =>{
                    fish.getComponent(cc.Button).enabled = false;
                    fish.node.off("click");
    ...
    private updateShoot(dt: number) {
        if (this.toggleAuto.isChecked || this.isTargetFish) {
            if (this.targetFish != null) {
                ...
                //cc.error("Find new fish")      
                this.countShoot = 0;        
                this.findTargetFishInWorld();
            }

            if (this.countShoot <= 1000) {    
                this.countShoot += 1;
            } else {
                this.countShoot = 0;        
                this.findTargetFishInWorld();
            }
            //cc.error("countShoot", this.countShoot)        
        }
    ...
    private findTargetFishInWorld() {
        ...
        //cc.error("findTargetFishInWorld", listFishActiveInWorld)
