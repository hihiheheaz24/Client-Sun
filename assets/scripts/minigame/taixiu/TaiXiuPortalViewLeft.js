...
var timeAll=60;
(function () {
    cc.TaiXiuPortalViewLeft = cc.Class({
        ...
        onLoad: function () {
            ...
            cc.TaiXiuControllerLeft.getInstance().setTaiXiuPortalView(this);
            cc.TaiXiuController.getInstance().setTaiXiuLeftView(this);
            ...
            this.connectHubTx();
            // this.luckyDiceHub = cc.TaiXiuController.getInstance().getHub(1);
            // this.onHubOpen();
        },
        ...
        onDestroy: function () {
            ...
            cc.TaiXiuControllerLeft.getInstance().setTaiXiuPortalView(null);
        },
        ...
        startTimer: function (remaining) {
            //cc.warn("startTimerLeft", remaining)
        ...
        updateTimer: function (time) {
            ...
            // cc.TaiXiuControllerLeft.getInstance().updateTimerInfoView(time);
        ...
        onHubMessage: function (response) {
            cc.TaiXiuController.getInstance().onMessage(1,response);
         },

        resumeGame(data){
            //cc.warn("RESUME LEFT", data)
            ...
            if(data.GameHistory){
                cc.TaiXiuControllerLeft.getInstance().updateSessionHistory(data.GameHistory);
            }
            if(data.BetSuccess){
                cc.TaiXiuControllerLeft.getInstance().updateBetInfoView(data.BetSuccess);
            } 
        },
                ...
                res.map(m => {
                    switch (m.M) {
                        ...
                        case cc.MethodHubOnName.GAME_HISTORY:
                            cc.TaiXiuControllerLeft.getInstance().updateSessionHistory(m.A[0]);
                            this.updateSessionData("GAME_HISTORY", m.A[0]);

                            //login roi -> moi goi
                            if (cc.LoginController.getInstance().getLoginState() && cc.TaiXiuControllerLeft.getInstance().getIsOpen()) {
                        ...
                        case cc.MethodHubOnName.BET_OF_ACCOUNT:
                            cc.director.getScheduler().schedule(function () {
                                cc.TaiXiuControllerLeft.getInstance().updateBetInfoView(m.A[0]);
                            }, this, 1, 0, 0.2, false);
                            break;
                        ...
                        case cc.MethodHubOnName.CORD_ACCOUNT_INFO:
                            ...
                            if (data.IsEventDragon) {
                                cc.TaiXiuControllerLeft.getInstance().activeEventPH(true);
                                cc.TaiXiuControllerLeft.getInstance().setUserCord(data.CordWin, data.CordLost);
                            } else {
                                cc.TaiXiuControllerLeft.getInstance().activeEventPH(false);
                            }
                            break;
                        //bet thanh cong
                        case cc.MethodHubOnName.BET_SUCCESS:
                            var data = m.A[0];
                            cc.TaiXiuControllerLeft.getInstance().updateBetInfoView(data);
                            //update lai balance
                            cc.BalanceController.getInstance().updateRealBalance(m.A[1]);
                            cc.BalanceController.getInstance().updateBalance(m.A[1]);

                            cc.DDNA.getInstance().betSummary(cc.DDNAGame.TAI_XIU, data.BetValue, cc.TaiXiuControllerLeft.getInstance().getSID());
                            break;
                        case cc.MethodHubOnName.WIN_RESULT:
                            ...
                            //dang bat che do Nan
                            if (cc.TaiXiuControllerLeft.getInstance().getIsNan()) {
                            ...
                            cc.director.getScheduler().schedule(function () {
                                //play fx win
                                cc.TaiXiuControllerLeft.getInstance().playEffectWin(data.Award);
                                //update lai balance
        ...
        onHubOpen: function () {
            cc.TaiXiuControllerLeft.getInstance().setIsOpen(true);
