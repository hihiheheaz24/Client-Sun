...
(function () {
    cc.MiniPokerView = cc.Class({
        ...
        onDestroy: function () {
            ...
            if (cc.sys.isNative) {
                var bundle = cc.assetManager.getBundle('minipoker');
               if(bundle)
                bundle.releaseAll();
            }
        },
            ...
            if (response.M !== undefined && response.M.length > 0) {
                ...
                switch (m.M) {
                    ...
                    case cc.MethodHubOnName.JOIN_GAME:
                        var data = m.A[0];
                        cc.MPSpinController.getInstance().setTotalTicket(data.totalTicket);
        ...
        activeArrow:function(isEnable)
        {
            this.arrowAnim.active = isEnable;
        },
        onScale: function() {
            if (!this.isScale) {
                this.isScale = true;
