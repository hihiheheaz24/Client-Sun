...
(function () {
    cc.DragonTigerView = cc.Class({
        ...
        onDestroy: function () {
            ...
            if (cc.sys.isNative) {
                var bundle = cc.assetManager.getBundle('dragonTiger');
               if(bundle)
                bundle.releaseAll();
            }
        },
        ...
        updateInfo: function (sessionInfo) {
            ...
            this.lbTotalBetRong.string = this.formatNumberSicbo(sessionInfo.TotalBetDragon);
            this.lbTotalBetHoa.string = this.formatNumberSicbo(sessionInfo.TotalBetTie);
            this.lbTotalBetHo.string = this.formatNumberSicbo(sessionInfo.TotalBetTiger);

            //this.updateTotalUser(totalUser);

        },
        formatNumberSicbo: function (strNumber) {
            return (parseInt(strNumber) == 0) ? "" : cc.Tool.getInstance().formatNumberSicbo(strNumber);
        },
        ...
        createNodeCardBack: function () {
            ...
            let spCardBackPosition = this.nodeCardStart.getChildByName("cardBack").position;
            nodeCardBack.position = spCardBackPosition;
            nodeCardBack.parent = this.nodeCardStart;
            nodeCardBack.rotation = -45;
            nodeCardBack.scale = 0.4;
            return nodeCardBack;
        },
        ...
        createNodeCardResult: function (type) {
            ...
            nodeCardResult.parent = this.nodeCardStart;
            nodeCardResult.scale = 0.7;
        ...
        listCardBack: function () {
            ...
            return [this.cardBackDragon, this.cardBackTiger];
        },
        //destroy card
        ...
        updateTimer: function (time) {
            ...
            switch (this.currentState) {
                ...
                case cc.DragonTigerState.END_BETTING:
                    ...
                    let progress = time / 25;
                    this.progressTimer.progress = 1 - progress;   
                    this.progressTimer.node.stopAllActions();
                    this.progressTimer.node.runAction(cc.repeat(
                        cc.sequence(
                            cc.delayTime(0.1),
                            cc.callFunc(()=>{
                                const time = this.progressTimer.progress + ((1/10)/25);
                                if(time > 1){
                                    this.progressTimer.node.stopAllActions();
                                    this.progressTimer.progress = 1;
                                } else {
                                    this.progressTimer.progress = time;   
                                }
                            }),
                        )
                        ,10)
                    ...
                    switch (m.M) {
                        ...
                        case cc.MethodHubOnName.TOTAL_WIN_MONEY:
                            ...
                            if (parseInt(dataTotalWin) > 0) {
                                setTimeout(function () {
                                    try {
                                        self.lbTotalUserWin.string = "+" + cc.Tool.getInstance().formatNumberSicbo(dataTotalWin);
        ...
        cardMoveTo: function (node, pos, duration) {
            ...
            let actionScaleTo = cc.scaleTo(duration, 0.7);
