...
(function () {
    cc.BauCuaBetView = cc.Class({
        ...
        onLoad: function () {
            ...
            this.betValue = 1000;//Khoi tao bet value 5k
            this.chipIndex = 2;
            this.betVals = [100, 500, 1000, 5000, 10000, 50000, 100000, 500000, 1000000];
            ...
            this.lstBtnBet = this.getListBtnBet([
                this.nodeDeer.getChildByName('btnBet').getComponent(cc.Button),
                this.nodeGourd.getChildByName('btnBet').getComponent(cc.Button),
                this.nodeRooster.getChildByName('btnBet').getComponent(cc.Button),
                this.nodeFish.getChildByName('btnBet').getComponent(cc.Button),
                this.nodeCrab.getChildByName('btnBet').getComponent(cc.Button),
                this.nodeLobster.getChildByName('btnBet').getComponent(cc.Button)
            ]);
            ...
            this.btnChips.forEach(function (btnChip) {
                self.nodeChipPress.push(btnChip.node.getChildByName('active'));
            });
        },
        getListBtnBet: function (arrNode) {
            let listBtn = [];
            arrNode.map(node => listBtn.push(node.getComponent(cc.Button)));
            return listBtn;
        },
        ...
        resetLbTotalBet: function () {
            ...
            // let nodeLstLabel = [
            //     this.lbTotalDeer.node,
            //     this.lbTotalGourd.node,
            //     this.lbTotalRooster.node,
            //     this.lbTotalFish.node,
            //     this.lbTotalCrab.node,
            //     this.lbTotalLobster.node
            // ];
            // nodeLstLabel.map(node => node.active = false);
        },
        ...
        nextBetClicked: function (event, data) {
            ...
            if (this.chipIndex > this._maxNextBet) {
                this.nodeGroupBet.runAction(cc.moveBy(0.1, cc.v2(-113, 0)));
                this._maxNextBet++;
                this._minPreBet++;
            }
            this.setBetValue(null, this.chipIndex);
        },
        ...
        setActiveButton: function () {
            for (var i = 0; i < this.nodeChipPress.length; i++) {
                this.btnChips[i].interactable = true;
                this.nodeChipPress[i].active = false;
                this.btnChips[i].node.runAction(cc.moveTo(0.1, cc.v2(this.btnChips[i].node.x, 0)));
            }
            ...
            this.btnChips[this.chipIndex].node.runAction(cc.moveTo(0.1, cc.v2(this.btnChips[this.chipIndex].node.x, 20)));
        ...
        setDefaultSfButton: function (node, type) {
            ...
            let moveBack = cc.moveTo(0.3, cc.v2(node.x, 30));
        ...
        onBetAgain: function (sender, unit) {
            ...
            if (logBet.length === 0) {
                cc.PopupController.getInstance().showMessageError("Chưa có dữ liệu của phiên trước.");
                return;
            }
        ...
        calcLogBet: function (logsBetSide, unit) {
            ...
            let listChip = [1000000, 500000, 100000, 10000, 5000, 1000];
        ...
        updateTotalUserBetSide: function (betSide, totalBet) {
            ...
            label.string = cc.Tool.getInstance().formatMoneyToKMB(totalBet);
        },
        //HubOn betOfAccount
        updateBetOfAccount: function (data) {
            data.map(betSide => {
                ...
                label.string = cc.Tool.getInstance().formatMoneyToKMB(betSide.BetValue);
            });
        },
        ...
        setStringValue: function (label, strNum) {
            label.string = strNum === 0 ? "" : cc.Tool.getInstance().formatMoneyToKMB(strNum);
        },
