 ...
 (function () {
    cc.BauCuaPlayer = cc.Class({
        ...
        onLoad: function () {
            ...
            //lbChip cua player
            this.lbChip = this.nodeInfo.getChildByName('NodeInfo').getChildByName('lbChip').getComponent(cc.LabelIncrement);
            //lbName cua player
        ...
        resetPlayerResultUI: function () {
            ...
            this.skeletonWin.node.active = false;
        },
        ...
        playerResultUI: function (isWin, amount) {
            ...
            if (isWin) {
                ...
                if (this.skeletonWin) {
                    this.skeletonWin.node.active = true;
                    this.skeletonWin.setAnimation(0, "animation", true);
                }
                this.lbWin.string = '+' + cc.Tool.getInstance().formatMoneyToKMB(amount);
        ...
        updateChip: function (chip) {
            ...
            this.lbChip.setValueText(cc.Tool.getInstance().formatMoneyToKMB(chip));
        },
        ...
        registerPlayer: function (accountInfo) {
            ...
            if (accountInfo.ServiceID) {
                //set name
                if (cc.LoginController.getInstance().getNickname() != accountInfo.NickName) {
                    // this.lbName.string = cc.Config.getInstance().formatName("[" + accountInfo.Country.trim().toUpperCase() + "] " + accountInfo.NickName.toLowerCase(), 7);
                    this.lbName.string = cc.Config.getInstance().formatName(accountInfo.NickName.toLowerCase(), 7);
                } else {
                ...
                //set name
                if (cc.LoginController.getInstance().getNickname() != accountInfo.NickName) {
                    // this.lbName.string = cc.Config.getInstance().formatName("[" + accountInfo.Country.trim().toUpperCase() + "] " + accountInfo.NickName.toLowerCase(), 10);
                    this.lbName.string = cc.Config.getInstance().formatName(accountInfo.NickName.toLowerCase(), 10);
                } else {
            ...
            this.lbChip.setValueText(cc.Tool.getInstance().formatMoneyToKMB(accountInfo.Balance));
