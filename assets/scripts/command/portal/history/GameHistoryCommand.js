...
(function () {
    ...
    /////////////////////////////////////
    var AllGameHistoryCommand;

    AllGameHistoryCommand = (function () {
        function AllGameHistoryCommand() {
        }

        AllGameHistoryCommand.prototype.execute = function (controller) {
            var url = 'api/History/GetAllGameHistory'; //12
            var params = JSON.stringify({
            ...
            return cc.ServerConnector.getInstance().sendRequestPOST(cc.SubdomainName.PORTAL, url, params, function (response) {
                var obj = JSON.parse(response);
                if (obj.ResponseCode === 1) {
                    return controller.onAllGameHistoryResponse(obj);
                }
            });
        };

        return AllGameHistoryCommand;

    })();

    cc.AllGameHistoryCommand = AllGameHistoryCommand;
}).call(this);
