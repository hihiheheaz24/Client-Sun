...
const { formatTime } = require('TTUtils');
cc.Class({
    ...
    updateDataByPageIndex(pageIndex){
        ...
        this.listItem.forEach((item,index)=>{
            if(result[index]){
                ...
                //item.getChildByName("layoutNN").getChildByName("lbSID").getComponent(cc.Label).string = "", //result[index].ServiceID;
    ...
    show () {
        this._super();
        let url = 'api/thanthu/GetBigWinner?top=100&type=' + cc.BigWinnerType.JACKPOT;
        cc.ServerConnector.getInstance().sendRequest(cc.SubdomainName.THAN_THU, url,  (response) =>{
            let obj = JSON.parse(response);
            this.onGetJackpotResponse(obj)
        });
